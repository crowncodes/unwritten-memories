...
function New-SilentStem {
    ...
    Write-Host "  Creating: $(Split-Path $OutputFile -Leaf) ($($DurationSeconds)s)"
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "    ✓ Created ($('{0:N0}' -f $size) KB)" -ForegroundColor Green
        return $true
    } else {
...
Write-Host "`n✓ Silent test stems generated!" -ForegroundColor Green
Write-Host ""
...
Write-Host "To replace with real music: Use music.V2 or Lyria API" -ForegroundColor Yellow
