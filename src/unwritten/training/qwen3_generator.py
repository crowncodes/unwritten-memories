...
class Qwen3DataGenerator:
    ...
    def __init__(self, config: Optional[TrainingConfig] = None):
        ...
        AppLogger.info(
            "Qwen3DataGenerator initialized (Master Truths v1.2)",
            data={
    ...
    def generate_with_qwen3(
        self, model: str, prompt: str, temperature: float = 0.85, max_tokens: int = 4000
    ) -> Optional[str]:
        """Call local Qwen3 model via Ollama"""

        # Auto-select timeout based on model
        if "8b" in model.lower():
            timeout = getattr(self.config, "timeout_8b", 120)
        elif "30b" in model.lower():
            timeout = getattr(self.config, "timeout_30b", 600)
        else:
        ...
        payload = {
            ...
            "options": {
                ...
                "top_p": 0.98,  # v1.6.2: Near-maximum for truly diverse sampling
                "top_k": 80,  # v1.6.2: Doubled for maximum vocabulary diversity (was 40)
            },
        }
        ...
        try:
            ...
            # DEBUG: Log prompt length and first 200 chars
            AppLogger.info(
                f"Sending to {model}",
                data={
            ...
            if not result:
                # DEBUG: Log full response to understand what Ollama returned
                AppLogger.warning(
                    ...
                    data={
                        ...
                        "context_length": len(response_data.get("context", [])) if response_data.get("context") else 0,
                    },
                )

            AppLogger.performance(
                f"Qwen3 generation ({model})",
                elapsed,
                data={"temperature": temperature, "response_length": len(result) if result else 0},
            )
    ...
    def generate_emotional_authenticity_batch(self, batch_size: Optional[int] = None) -> List[Dict]:
        """
...
Return ONLY valid JSON array."""
    ...
    def generate_dramatic_irony_batch(self, batch_size: Optional[int] = None) -> List[Dict]:
        """
...
Return ONLY valid JSON array."""
    ...
    def generate_tension_building_batch(self, batch_size: Optional[int] = None) -> List[Dict]:
        """
...
Return ONLY valid JSON array."""
    ...
    def generate_memory_resonance_batch(self, batch_size: Optional[int] = None) -> List[Dict]:
        """
...
Return ONLY valid JSON array."""
    ...
    def generate_personality_trait_batch(self, batch_size: Optional[int] = None) -> List[Dict]:
        """
...
Return ONLY valid JSON array."""
    ...
    def generate_relationship_scoring_batch(self, batch_size: Optional[int] = None) -> List[Dict]:
        """
  ...
  "strengths": ["What works well"]
    ...
    def _parse_json_response(self, response_text: str, data_type: str) -> List[Dict]:
        ...
        try:
            ...
            # Try to find JSON bounds (object or array)
            # v1.6.3: Handle both single object {...} and array [{...}]
            ...
            # Fix: Remove leading + signs on numbers (JSON doesn't allow +0.3, only 0.3 or -0.3)
            cleaned = re.sub(r":\s*\+(\d)", r": \1", cleaned)  # ": +0.3" -> ": 0.3"

            # Fix: Replace problematic Unicode with ASCII equivalents
            # v2.2: Handle em-dashes and smart quotes BEFORE removing non-ASCII
            ...
            # Handle smart quotes (for contractions and quotes)
            ...
            cleaned = cleaned.replace("'", "'")  # Catch any remaining
            cleaned = cleaned.replace("'", "'")
            cleaned = cleaned.replace(""", '"')
            cleaned = cleaned.replace(""", '"')
            
            # Handle other special characters
            cleaned = cleaned.replace("×", "*")  # Multiplication → asterisk
            cleaned = cleaned.replace("…", "...")  # Ellipsis
            
            # NOW remove remaining non-ASCII (after all conversions above)
            cleaned = re.sub(r"[^\x00-\x7F]+", " ", cleaned)  # Replace with space

            # Fix: Broken field names after Unicode removal (e.g., "demon,"demonstrates" -> "demonstrates")
            cleaned = re.sub(r'"\w+,"\s*"', '"', cleaned)  # Remove broken partial fields

            # Fix: Try to handle nested quotes by escaping them
            # This is a heuristic - look for patterns like: "text":"\"quote\""
            # and convert to: "text":"'quote'"
            cleaned = re.sub(r'(:\s*")(\\"[^"]*\\")(")', r"\1\2\3", cleaned)

            # Save failed response for debugging
            if len(cleaned) > 50000:
                AppLogger.warning(
                    f"Very large response for {data_type}", data={"size": len(cleaned)}
                )
            ...
            try:
                ...
                # Attempt to salvage valid objects from incomplete array
                if "[{" in cleaned and "Expecting" in str(e):
                    AppLogger.warning(f"Attempting to salvage partial JSON for {data_type}")
                    # Find position before last incomplete object
                    # Look for the last complete '}'
                    last_complete = cleaned.rfind(
                        "}", 0, e.pos if hasattr(e, "pos") else len(cleaned)
                    )
                    if last_complete > 0:
                        # Truncate and close array
                        salvaged = cleaned[: last_complete + 1] + "]"
                        try:
                            data = json.loads(salvaged)
                            AppLogger.info(
                                f"✓ Salvaged {len(data)} complete objects from partial JSON"
                            )
                        except Exception as salvage_error:
                            AppLogger.error("Salvage attempt failed", salvage_error)
            ...
            if not isinstance(data, list):
                AppLogger.error(
                    f"Expected list for {data_type}, got {type(data).__name__}",
                    ValueError("Invalid response format"),
                )
                # Save raw response for debugging
                debug_file = (
                    self.output_dir
                    / f"failed_{data_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
                )
                with open(debug_file, "w", encoding="utf-8") as f:
                    f.write(response_text)
                AppLogger.info(f"Saved failed response to {debug_file}")
                return []

            AppLogger.info(f"Successfully parsed {data_type} batch", data={"count": len(data)})
            return data

        except json.JSONDecodeError as e:
            AppLogger.error(f"JSON parsing failed for {data_type}", e)
            AppLogger.error(f"Parse error at: line {e.lineno}, column {e.colno}", None)
            # Save raw response for debugging
            debug_file = (
                self.output_dir
                / f"failed_{data_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
            ...
            try:
                with open(debug_file, "w", encoding="utf-8") as f:
                    f.write(f"ERROR: {e}\n")
                    f.write(f"Location: line {e.lineno}, column {e.colno}\n")
                    f.write(f"\n{'='*80}\n")
                    f.write(f"ORIGINAL RESPONSE:\n")
                    f.write(f"{'='*80}\n")
                    f.write(response_text)
                    f.write(f"\n\n{'='*80}\n")
                    f.write(f"CLEANED FOR PARSING:\n")
                    f.write(f"{'='*80}\n")
                    f.write(cleaned)
                AppLogger.info(f"Saved failed response to {debug_file}")
            except:
    ...
    def save_batch(
        self, data: List[Dict], data_type: str, batch_number: int, timestamp: str
    ) -> str:
        ...
        try:
            ...
            AppLogger.info(
                f"Saved {data_type} batch", data={"file": filename, "samples": len(data)}
            )
            return str(filepath)
    ...
    def run_production_cycle(self, duration_hours: int = 24) -> Dict[str, List[Dict]]:
        ...
        AppLogger.info(
            f"Starting {duration_hours}h production (Master Truths v1.2)",
            data={
                "daily_target": self.config.total_daily_target,
                "quality_thresholds": self.config.quality_thresholds,
            },
        ...
        try:
            ...
            AppLogger.success(
                "Production cycle complete (Master Truths v1.2)",
                data={
