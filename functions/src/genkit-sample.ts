...
// Use the plugin helper to select a model explicitly
...
// Load env from functions/.env and (fallback) repo root .env for local dev
loadEnv({ path: path.resolve(process.cwd(), ".env") });
loadEnv({ path: path.resolve(process.cwd(), "..", ".env"), override: false });

const ai = genkit({
  plugins: [
    // Prefer GOOGLE_GENAI_API_KEY; fallback to GEMINI_API_KEY for local dev
...
// Register additional flows in Dev UI
import "./location_flows";
import "./flows/dialogue";
import "./flows/music";
import "./flows/lyria";
import "./flows/story";
